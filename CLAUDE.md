...
│   ├── server/            # Express API
│   │   ├── services/      # card-news, note-card-news, notes, r2, gemini, content, ga4, supabase, weekly-report, cron 등
│   │   └── routes/        # exam, question, card-news, notes, content, analytics, weekly-reports 등
│   ├── supabase/migrations/  # SQL 마이그레이션 (weekly_reports 등)
│   └── src/               # React + Vite 프론트엔드
│       ├── features/      # exam, question, generator, card-news, notes, content, analytics(대시보드+주간리포트)
...
SUPABASE_URL=https://uonznnypdnerdigfyfci.supabase.co  # 주간 리포트 저장
