...
- **ExamSelector 모바일 노출 + 액션 라벨 (2026-05-26)**: 기존 `hidden sm:block` 래퍼 제거해 **모든 폭에서 노출** (다중 시험 플랫폼이라 모바일에서도 시험 전환이 핵심 동선인데 sm 미만에서 안 보였던 버그). 버튼 라벨도 현재 시험명(`🏛️ 9급 공무원`) 대신 **`📚 다른 시험 ▼`** 액션 텍스트 + 앰버 2px 보더 + 앰버 동그라미 chevron 뱃지로 교체 — "선택 가능한 드롭다운"임을 시각적으로 명확히. 현재 시험은 드롭다운 안에서 ● 마커 + 배경 하이라이트 + auto-expand로 강조. 컨텍스트는 페이지 Hero/breadcrumb/cross-sell pill에 이미 존재해서 버튼에서 빼도 손실 없음. 접근성은 `aria-label`에 `"현재 9급 공무원 — 다른 시험 보기"` 포함
...
- **정답 후 학습 UX (Drawer + 서버 렌더 학습자료, 2026-05-05 / 2026-05-29 개정)**: 정답 확인 후 카드 외부에 amber 노트 박스 — `📝 관련 요약노트 (N편)` → **NoteDrawer** (우측 80vw 슬라이드, 모바일 100%). drawer는 모든 노트 일렬 펼침 + `note-content` 클래스로 globals.css 노트 스타일 재사용 + `expandAllDetails()` 정규식으로 모든 `<details>` 강제 open (표/타임라인 즉시 보임). ESC + backdrop close, body scroll lock. 핵심 파일: `web/components/{NoteDrawer, QuestionWithRelatedPanel, QuestionSEOContent}.tsx` + QuestionCard에 `hideRelatedNotes`/`hideYouTubeInCard` props. 한능검만 적용 (CBT는 v2 별도). 상세 [memory/note_drawer_action_box_v1.md]
  - **2026-05-29 — `QuestionSEOContent` 서버 렌더 복원 (AdSense "low value" 대응)**: 기존엔 `QuestionWithRelatedPanel`(`"use client"`) 안에서 `{answered && ...}` 게이트 + `bare` 모드로 렌더돼 **정답 클릭 전엔 DOM에 아예 없었음** → 봇/AdSense 리뷰어가 문제 페이지를 "질문+선지 4개"만 있는 빈 페이지로 인식 (low-value 반려 원인 후보). 이제 `QuestionSEOContent`를 `page.tsx`(서버 컴포넌트)에서 **직접 렌더** → 시대 배경·해설·관련 기출이 정답 클릭과 무관하게 **항상 서버 HTML에 존재** (크롤 가능). 화면엔 `📚 학습 자료 펼쳐보기` 접힌 `<details>` 한 줄 (퀴즈 안 까짐, 펴면 전체 자료 — 원래 설계 의도 복원). `QuestionWithRelatedPanel`에서 중복 inline expand 제거, `bare` prop 제거. **주의**: 시대 배경 텍스트는 같은 시대 문제끼리 공유 → 단독으로 "고유 콘텐츠" 요건 충족엔 부족, near-duplicate 우려 있음. AdSense 정책센터는 특정 URL 없는 일반 통지(최소/고유/빈약 콘텐츠)라 thin 프로그래매틱 페이지 비중 정리가 별도 과제로 남음.
...
```
R2_PUBLIC_URL=
NEXT_PUBLIC_SUPABASE_URL=https://ptyjifdidulnkuunkswv.supabase.co
...
SUPABASE_URL=https://ptyjifdidulnkuunkswv.supabase.co  # 주간 리포트만 사용 (그 외는 R2 JSON)
...
### AdSense 6차 반려 후속 — /*/*/exam 전체 봇 차단 (2026-06-27)
AdSense "가치 낮은 콘텐츠" 6번째 반려 (재심사 2026-06-26). 6/9 noindex가 배포·동작했음에도(변리사=noindex 라이브 확인) 또 반려된 핵심 원인: **noindex는 AdSense 크롤·평가를 막지 못함** — 리뷰어가 태그 무시하고 홈 OtherExamsTree 내부링크로 547 ExamType + 100만 CBT 양산 표면을 그대로 렌더·평가. 6/10에 남긴 교훈("크롤 감소는 robots Disallow")을 ClaudeBot에만 적용하고 Googlebot/AdSense엔 안 건 빈틈.
- **처방**: `web/app/robots.ts` `sharedDisallow`에 `/*/*/exam` 추가 → CBT·공무원 기출/문제 100만+ URL을 **전체 봇** 차단 (기존 ClaudeBot 전용 → 전 봇 확대). 이미 6/9 게이트로 전부 noindex(`civilQuestionMeta`/`civilExamListMeta` always noindex)라 **검색 순위 손실 0**.
- `/*/*/notes`는 ClaudeBot 전용 유지 — 통합 노트 중 23개 수동 과목은 실제 색인 콘텐츠라 Googlebot엔 열어둠(AdSense에 보여줄 진짜 콘텐츠라 차단하면 역효과).
...
- **네이버 vs 구글 SEO 차이 정리 (memory)**: 우리 작업이 양쪽 다 통한 결정적 요인 — ① Next.js SSG (네이버 봇 JS 약한데 정적 HTML 색인) ② 메타/타이틀 정확 키워드 매칭 (네이버 LDA는 의미보다 단어 매칭) ③ 87 노트 + 1,900 기출 + 22 blog 콘텐츠 양 임계점 돌파 ④ sitemap.xml + 서치어드바이저 등록. 네이버 외부 사이트 천장이 낮아 다음 단계는 블로그/카페 외부 인용 (수동 운영부터). 상세 [memory/seo_naver_vs_google.md].
...
### 노트 보강 pilot — 4과목 한능검 수준화 검토 중 (2026-05-07, 진행)
**범위**: 행정법(28단원) + 행정학(15) + 국어(9) + 영어(9) = 61단원 → 단원당 ~20K자로 보강. 9급 시장 1순위가 일반행정 직렬이라 컨텐츠 ROI 최대.

**비교 결과** (`_pilot/output/`):
- `admin-law-t1-gemini-flash.html` — Gemini 2.5 Flash 단순 텍스트 확장 (시드 1,978자 → 42,398자). 분량은 풍부하지만 reviewer 에이전트가 **critical 사실오류 5건 + warning 다수** 검출:
  - 행정절차법 제14조 ↔ 제15조 혼동 (도달주의 + 공고 14일은 제15조)
  - 대판 2017두45131 판시 결론을 거꾸로 인용
  - 행정기본법 제19조에 없는 "예측 불가 손실 보상" 조항 hallucinate
  - 사건번호 2건(`2008두15024`, `2007두2634`) hallucinate 의심 (DB 검색 미확인)
  - 부관·하자승계·중대명백설 등 9급 빈출 토픽 일부 누락
- `admin-law-t1-claude.html` — Claude 직접 작성 + WebSearch로 모든 조문/판례 검증 (20,901자, 한능검 평균 부합). 5건 critical 모두 회피, 검증된 판례만 인용.

**스크립트**: `_pilot/expand-civil-topic.mjs` — Gemini 2.5 Flash 1회용 확장기 (시드 + 한능검 레퍼런스 → HTML).

**다음 결정** (집에서 이어서):
- A. Claude 직접: 검증 OK이지만 단원당 ~$0.5, 61단원 ≈ $30 / 10시간
- B. Gemini 생성 → reviewer 자동 fact-check + 수정 절충안: 단원당 ~$0.05 + 검수
- C. 검증 인프라(공식 법령 DB 검색 강제 + 출력 후 fact-check 스크립트) 세팅 후 batch
- 현재 미결정 — pilot 결과만 보고 향후 방향 선택 예정. **데이터 소스를 기출문제 기반으로 재설계** 검토 (한능검 원래 방식 = 출제 데이터 추출 → 빈출 주제별 polish)
