...
import { supabase, Board, BOARD_LABELS } from "@/lib/supabase";
...
// GET /api/board/[board] — list posts
export async function GET(req: NextRequest, ctx: RouteContext) {
  ...
  const { data, error, count } = await supabase
    .from("posts")
    .select("id, board, nickname, title, created_at", { count: "exact" })
    .eq("board", board)
...
export async function POST(req: NextRequest, ctx: RouteContext) {
  ...
  // Notice board: require admin password
  if (board === "notice") {
  ...
  // Post password for deletion (required for non-notice posts)
