...
/**
 * Server-rendered study material, inlined into the initial HTML of every
 * question page so search crawlers (and AdSense reviewers who expand it) see
 * substantive unique content — the interactive QuestionCard is client-only.
 *
 * Collapsed by default inside a <details>: natural quiz UX (attempt first,
 * reveal answer/explanation on demand) and not spoiled. The summary label
 * advertises the content so a glancing reviewer sees the page offers a study
 * guide. The key point is that this renders SERVER-SIDE — present in the HTML
 * regardless of whether the user has answered.
 */
export default function QuestionSEOContent({ exam, question, related }: Props) {
  ...
  return (
    <section className="mt-8" aria-label="문제 학습 자료">
        ...
        <div className="border-t border-slate-100 px-5 py-5 space-y-6 text-[14px] leading-[1.75] text-slate-700">
          ...
          <div>
            ...
            <p className="mb-2">
              ...
              {correctText ? <> — &ldquo;{correctText}&rdquo;이다.</> : <> 이다.</>}
            </p>
